...
function EditBlog() {
  ...
  const [defaultImage, setDefaultImage] = useState(""); // Set the default image URL here
  ...
  const updateData = () => {
    ...
    setInputData({
      jobTitle: getData.blog_Heading || "",
      description: getData.blog_Desciption || "",
    });
    setDefaultImage(`http://192.168.1.43:2001/${getData.blogImage}` || "");
  };
  ...
  const submitAllData = () => {
    ...
    formData.append("blogHeading", inputData.jobTitle);
    formData.append("blogDescription", inputData.description);

    axios
      .put(`${baseUrl}updateBlog/${location.state.id}`, formData)
      .then((response) => {
